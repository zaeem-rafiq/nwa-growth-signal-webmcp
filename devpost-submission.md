...
The research task has three distinct steps, each with a clear contract, plus one question every returning researcher asks first. `search_planning_cases` filters 5 verified records by municipality, procedural status, residential relevance, and pending government action. `inspect_case_record` returns one filing-level record with its context and official sources. `stage_source_backed_brief` places 1 to 5 selected records into the visible briefing workspace. `list_status_changes` answers “what moved since the last check?”: it returns each filing whose verified status label changed between August 25 and September 2 with the official source consulted on each date, and can include the unchanged filings with their notes. The agent receives typed results from the same dated data the person sees instead of inferring status from page wording or clicking by coordinates.
...
The release has 89 passing dependency-free tests, a deterministic benchmark, and a fail-closed freshness gate. The team used Codex and Claude Code for implementation, review, testing, and submission work; Hyperagent for municipal research and editorial work; and Google Gemini for the approved demo narration.
...
- 89 passing tests, all 4 tools exercised, input validation, atomic registration, cancellation support, and a fail-closed freshness gate.
...
7. Ask: “Which filings changed status between the August 25 and September 2 verifications?” Verify that `list_status_changes` returns exactly `RZ26-00419` (Tabled to Recommended) and `RZ26-00511` (Scheduled to Recommended), each with the Rogers outcome-table URL for both dates, and that the receipt row reads `2 filing statuses changed · 6 unchanged.`
