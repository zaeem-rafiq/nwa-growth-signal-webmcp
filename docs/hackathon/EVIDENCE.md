...
- **Artifact:** `site/`, the 97 dependency-free tests, the release benchmark, the held-out historical benchmark, and candidate viewport/browser evidence.
...
- **Observed result:** The current automated suite passes 97/97. The candidate passed browser verification at 375, 768, and 1440 pixels with no horizontal overflow or console warnings and errors.
...
A read-only audit of `site/`, `scripts/`, and `tests/` on September 2 found no injection sink, no network or storage path beyond the same-origin `cases.json` fetch, no secret or personal data in the tree or history, and bounded, input-free receipts. It rated one item Medium (no Content-Security-Policy or framing protection on the host) and five Low. The release that followed adds `site/_headers` (strict same-origin CSP, `frame-ancestors 'none'`, `X-Frame-Options: DENY`, permissions policy), stops handler errors from echoing caller input (a typed `NOT_FOUND` code with the receipt `Call rejected: unknown record.`), enforces the municipal-domain allowlist at render time so a non-official URL is shown as text rather than a link, and pins CI actions to commit SHAs. The page was exercised under the new policy with zero violations before deployment. The documented agent-staging behavior (an agent call replaces the person's in-progress selection, attributed on screen) is unchanged.
