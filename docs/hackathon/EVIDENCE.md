...
`list_status_changes`, added September 2 after the video was recorded, reads that same history and returns the filings whose status moved between the previous check and the current one: `RZ26-00419` (Tabled to Recommended) and `RZ26-00511` (Scheduled to Recommended), each with the official source checked on both dates. With `changed_only: false` it also lists the six unchanged filings and their notes, including `FP26-0005` absent from the latest reissued agenda and `RZ26-00345` gone from the outcome table but present as a matching ordinance on the August 11 council agenda without a published result. Its receipt row reads `2 filings changed since 2026-08-25.`
...
- **Artifact:** `site/`, the 99 dependency-free tests, the release benchmark, the held-out historical benchmark, and candidate viewport/browser evidence.
...
- **Observed result:** The current automated suite passes 99/99. The candidate passed browser verification at 375, 768, and 1440 pixels with no horizontal overflow or console warnings and errors.
...
## Security hardening, September 2, 2026

A read-only audit of `site/`, `scripts/`, and `tests/` on September 2 found no injection sink, no network or storage path beyond the same-origin `cases.json` fetch, no secret or personal data in the tree or history, and bounded, input-free receipts. It rated one item Medium (no Content-Security-Policy or framing protection on the host) and five Low. This release adds `site/_headers` (strict same-origin CSP, `frame-ancestors 'none'`, `X-Frame-Options: DENY`, permissions policy), stops handler errors from echoing caller input (a typed `NOT_FOUND` code with the receipt `Call rejected: unknown record.`), enforces the municipal-domain allowlist at render time so a non-official URL is shown as text rather than a link, and pins CI actions to commit SHAs. Before deployment the page was served locally with the same headers injected on every response and driven by Playwright with a stub model context: four tools exposed and exercised, the staging and not-found receipts rendered, the web font loaded, zero `securitypolicyviolation` events, and an empty console. The documented agent-staging behavior (an agent call replaces the person's in-progress selection, attributed on screen) is unchanged.

### Production verification, September 2, 2026 (hardened release)

After PR #18 merged as `2967d20` and `site/` was deployed to the production branch, https://nwa-growth-signal-webmcp.pages.dev/ was checked directly:

- Every response, including the two PDFs, now carries `Content-Security-Policy: default-src 'self'; script-src 'self'; style-src 'self'; img-src 'self'; font-src 'self'; connect-src 'self'; object-src 'none'; base-uri 'none'; form-action 'none'; frame-ancestors 'none'`, `X-Frame-Options: DENY`, `X-Content-Type-Options: nosniff`, `Referrer-Policy: strict-origin-when-cross-origin`, and `Permissions-Policy: camera=(), microphone=(), geolocation=()`. The `_headers` file itself is not served; that path falls back to the page.
- `index.html` (at `/`), `cases.json`, `core.js`, `webmcp.js`, `app.js`, and `styles.css` served by production have the same SHA-1 as the merged source.
- Playwright with a stub `document.modelContext` on the live URL: four tools exposed, `list_status_changes` returned `RZ26-00419` and `RZ26-00511`, a three-filing brief staged, an unknown case ID returned `NOT_FOUND` with the receipt `Call rejected: unknown record.`, the web font loaded, zero `securitypolicyviolation` events, an empty console, and no horizontal overflow at 375 pixels.
- The three-page hackathon PDF opened and rendered in Google Chrome's built-in viewer under the new headers.
- Fourth native run in the ChatGPT desktop browser against this release: `WebMCP ready · 4 tools exposed` with read-only hints true, true, false, true; `list_status_changes` returned the two changed Rogers filings with both dated sources; the demo task staged `RZ26-0041`, `RZ26-00419`, and `RZ26-00511` with `review_required: true` and current freshness; `inspect_case_record` with an unknown ID returned `Unknown planning record.` and the receipt row `Call rejected: unknown record.`; the console error and warning list was empty, with no Content Security Policy messages. The agent did not mark the brief reviewed, publish, send, or save. The verbatim report is kept outside the repository at `demo/native-run-2026-09-02d.md`.
